        ...
        <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel>Name des Sponsors*</FormLabel><FormControl><Input placeholder="Firmenname GmbH" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="level" render={({ field }) => (<FormItem><FormLabel>Sponsoring-Level*</FormLabel><FormControl><Input placeholder="Hauptsponsor, Partner, etc." {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="logoFile" render={({ field: { onChange, onBlur, name, ref } }) => (<FormItem><FormLabel>Logo* (max 5MB, JPG/PNG/GIF/SVG)</FormLabel><FormControl><Input type="file" accept="image/jpeg,image/png,image/gif,image/svg+xml" onBlur={onBlur} name={name} ref={ref} onChange={(e) => onChange(e.target.files)} /></FormControl><FormDescription>{editingSponsor ? 'Lassen Sie das Feld frei, um das aktuelle Logo beizubehalten. Ein neues Logo ist nur beim Erstellen erforderlich.' : 'Ein Logo ist für neue Sponsoren erforderlich.'}</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="displayOrder" render={({ field }) => (<FormItem><FormLabel>Anzeigereihenfolge</FormLabel><FormControl><Input type="number" {...field} value={field.value || 0} /></FormControl><FormDescription>Niedrigere Zahlen werden zuerst angezeigt.</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="dataAiHint" render={({ field }) => (<FormItem><FormLabel>Bild KI-Hinweis (Optional)</FormLabel><FormControl><Input placeholder="z.B. company logo, tech brand" {...field} value={field.value || ''} /></FormControl><FormDescription>1-2 Stichworte für KI-Bildgenerierung.</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="websiteUrl" render={({ field }) => (<FormItem><FormLabel>Webseite URL (Optional)</FormLabel><FormControl><Input type="url" placeholder="https://www.firmenname.de" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
