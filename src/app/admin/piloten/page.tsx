        ...
        <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel>Name des Piloten*</FormLabel><FormControl><Input placeholder="Max Mustermann" {...field} /></FormControl><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="profileSlug" render={({ field }) => (<FormItem><FormLabel>Profil Slug (Optional)</FormLabel><FormControl><Input placeholder="max-mustermann" {...field} /></FormControl><FormDescription>Wird für die URL des Profils verwendet. Wenn leer, wird kein Profil erstellt.</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="imageFile" render={({ field: { onChange, ...rest } }) => (<FormItem><FormLabel>Bild {editingPilot ? 'ersetzen' : 'hochladen'} (Optional)</FormLabel><FormControl><Input type="file" accept="image/jpeg,image/png,image/gif" onChange={(e) => onChange(e.target.files)} {...rest} /></FormControl><FormDescription>{editingPilot ? 'Lassen Sie das Feld frei, um das aktuelle Bild beizubehalten.' : 'Wählen Sie eine Bilddatei.'}</FormDescription><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="bio" render={({ field }) => (<FormItem><FormLabel>Bio (Optional)</FormLabel><FormControl><Textarea placeholder="Kurze Beschreibung des Piloten..." {...field} /></FormControl><FormMessage /></FormItem>)} />
                <FormField control={form.control} name="achievements" render={({ field }) => (<FormItem><FormLabel>Erfolge (Optional, getrennt durch | )</FormLabel><FormControl><Textarea placeholder="Erster Platz Meisterschaft X|Zweiter Platz Rennen Y" {...field} /></FormControl><FormDescription>Jeden Erfolg mit einem | trennen.</FormDescription><FormMessage /></FormItem>)} />
