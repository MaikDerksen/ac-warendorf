...
import { ArrowLeft, UserCog, Edit, Trash2, Loader2, List, PlusCircle } from 'lucide-react';
...
const boardMemberFormSchema = z.object({
  ...
  })).min(1, "Es muss mindestens eine Rolle angegeben werden."),
...
export default function AdminVorstandPage() {
  ...
  const form = useForm<BoardMemberFormValues>({
    ...
    defaultValues: { name: "", email: "", roles: [{ role: "", term: "" }], description: "", imageFile: undefined, order: 99 },
  });
  ...
  const handleCancelEdit = () => {
    ...
    form.reset({ name: "", email: "", roles: [{ role: "", term: "" }], description: "", imageFile: undefined, order: 99 });
  };
  ...
  async function onSubmit(data: BoardMemberFormValues) {
    ...
    formData.append('order', String(data.order || 99));
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                  {fields.map((item, index) => (
                    <div key={item.id} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-4 items-start border-b pb-4 mb-4">
                      <FormField
                        control={form.control}
                        name={`roles.${index}.role`}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Rolle / Funktion {index + 1}*</FormLabel>
                            <FormControl>
                      ...
                      >
                        ...
                        <span className="sr-only">Rolle entfernen</span>
                      </Button>
                    </div>
                  ))}
                   <Button
                ...
                <FormField 
                    ...
                    render={({ field: { onChange, onBlur, name, ref }}) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input type="file" accept="image/jpeg,image/png,image/gif" onBlur={onBlur} name={name} ref={ref} onChange={(e) => onChange(e.target.files)} />
                            </FormControl>
                            <FormDescription>{editingMember ? 'Lassen Sie das Feld frei, um das aktuelle Bild beizubehalten.' : 'Wählen Sie eine Bilddatei.'}</FormDescription>
                            <FormMessage />
                        </FormItem>
                    )}
                />
