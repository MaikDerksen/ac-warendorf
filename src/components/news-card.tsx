  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-out">
        ...
        <Link href={`/news/${article.slug}`} className="block aspect-video relative">
            {article.heroImageUrl ? (
                ...
                <div className="w-full h-full bg-muted flex items-center justify-center">
                    <Newspaper className="h-24 w-24 text-foreground/20" />
                </div>
            )}
        </Link>
      </CardHeader>
      <CardContent className="p-6 flex-grow bg-secondary dark:bg-card">
        ...
        <p className="text-sm text-secondary-foreground dark:text-muted-foreground mb-3 line-clamp-3">{article.excerpt}</p>
        <div className="text-xs text-secondary-foreground dark:text-muted-foreground flex items-center space-x-2 mb-1">
          <CalendarDays className="h-3.5 w-3.5" />
          <span>{formattedDate}</span>
        </div>
        {article.categories && article.categories.length > 0 && (
          <div className="text-xs text-secondary-foreground dark:text-muted-foreground flex items-center space-x-1">
      ...
      <CardFooter className="p-6 pt-0 bg-secondary dark:bg-card">
        <Button asChild variant="outline" className="w-full hover:bg-accent hover:text-accent-foreground dark:border-input dark:bg-background dark:hover:bg-accent dark:hover:text-accent-foreground">
