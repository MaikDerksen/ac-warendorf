...
import { Loader2, Users, Save, Image as ImageIcon, UploadCloud } from 'lucide-react';

const MAX_CONTACT_PERSONS = 4;
const NONE_OPTION_VALUE = "--none--";

export function HomepageSettingsManager() { // Renamed component
    ...
    async function fetchData() {
      ...
      try {
        const [membersRes, settingsRes] = await Promise.all([
          ...
          fetch('/api/admin/settings/homepage-contacts') // This API now returns full settings
        ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/settings/homepage-contacts', { // API path kept for now
        method: 'POST',
      ...
      // Update current URLs if new ones were saved
      ...
      // Clear file inputs visually (hacky but common)
      ...
      <CardContent className="space-y-8">
        ...
        {selectedContactIds.map((selectedId, index) => (
          <div key={index} className="space-y-2">
            ...
            <Select
              value={selectedId || NONE_OPTION_VALUE} // Ensure placeholder shows if ""
